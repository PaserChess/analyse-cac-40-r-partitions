...
def predict_returns_ml(prices):
    """
    VERSION 'NAÏVE' (SANS XGBOOST) :
    On remplace l'IA par une simple moyenne historique.
    Cela permet de vérifier si l'instabilité venait de l'IA.
    """
    # Calcul des rendements quotidiens
    returns = prices.pct_change().dropna()
    
    # Prédiction = Moyenne historique annualisée (Simplement ce que l'action a fait en moyenne sur 3.5 ans)
    # C'est l'approche standard de Markowitz
    predicted_means = returns.mean() * 252
