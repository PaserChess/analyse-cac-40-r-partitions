...
SEUIL_FILTRE = 0.01    # Conviction Filter
...
all_tickers_ever = sorted(list(set([t for yr in raw_history.values() for t in yr] + ['CAC.PA'])))
print("2. DOWNLOADING DATA...")
try:
    data = yf.download(all_tickers_ever, start="2010-01-01", auto_adjust=True)['Close'].ffill()
    prices_bench = data['CAC.PA']
...
def optimize_black_litterman(prices_train, tau=tau): 
    ...
    # --- MODIFICATION ICI : OPTIMISATION DU SORTINO RATIO ---
    # On convertit le taux sans risque annuel en taux quotidien
    rf_daily = RISK_FREE_RATE / 252
    
    def neg_sortino_bl(w):
        # 1. Rendement attendu du portefeuille (BL annualisé)
        p_ret_annual = np.sum(bl_returns * w)
        
        # 2. Calcul du Downside Risk (sur l'historique d'entraînement des actifs)
        # Rendement quotidien du portefeuille sur la période d'entraînement
        port_returns_daily = returns.dot(w)
        
        # On ne garde que les rendements inférieurs au taux sans risque quotidien
        downside_returns = port_returns_daily - rf_daily
        downside_returns = np.minimum(downside_returns, 0.0)
        
        # Volatilité à la baisse (annualisée)
        downside_deviation_annual = np.sqrt(np.mean(downside_returns ** 2)) * np.sqrt(252)
        
        if downside_deviation_annual < 1e-6: 
            return 0 
        
        # Sortino Ratio négatif (pour la minimisation)
        return - (p_ret_annual - RISK_FREE_RATE) / downside_deviation_annual
    ...
    try:
        # On remplace neg_sharpe_bl par neg_sortino_bl
        res = minimize(neg_sortino_bl, [1/n_assets]*n_assets, method='SLSQP', bounds=bounds, constraints=cons)
...
def run_scenario(scenario_id, name, start_year, end_year, use_filter):
    ...
    while current_year <= end_year:
        for m in [1, 4, 7, 10]: #les mois par lequels on commence
    ...
    # --- NEW: Variable pour conserver la dernière allocation ---
    ...
    for start_test, end_test in periods:
        ...
        if alloc is not None:
            ...
            # --- NEW: CALCUL DES FRAIS EXACTS ---
            if prev_alloc.empty:
                # Tout premier investissement : on achète l'intégralité du portefeuille
                turnover = 1.0 
            else:
                # Calcul de la différence absolue des poids
            ...
            if len(common_idx) > 0:
                ...
                # --- NEW: Prélèvement des frais à la source le 1er jour du trimestre ---
            ...
            # --- NEW: On sauvegarde la composition pour le prochain tour ---
            prev_alloc = alloc.copy()
        else:
            prev_alloc = pd.Series(dtype=float) # Passage 100% Cash
...
# --- 6. EXECUTION ---
scenarios_list = [
    {"id": 1, "years": (2016, 2022), "filter": True,  "desc": f"7 Years | CONVICTION BL+SIM+XGBOOST (>12%)"},
    {"id": 2, "years": (2023, 2026), "filter": True,  "desc": f"3 Years  | CONVICTION BL+SIM+XGBOOST (>12%)"},
]
...
if alloc_base is not None:
    ...
    if alloc_conviction is not None:
        ...
        # Calcule le Sortino historique sur le portefeuille final pour l'affichage
        downside_diff = train_port_26 - (RISK_FREE_RATE / 252)
        p_downside_vol = np.sqrt(np.mean(np.minimum(downside_diff, 0.0) ** 2)) * np.sqrt(252)
        sortino_ratio = (p_ret_26 - RISK_FREE_RATE) / p_downside_vol if p_downside_vol > 1e-6 else 0

        print("\nRISK DASHBOARD (Est. over 3.5 years)")
        print("-" * 50)
        print(f"   Expected Return (Hist): {p_ret_26*100:.2f}%")
        print(f"   Annual Volatility     : {p_vol_26*100:.2f}%")
        print(f"   Sortino Ratio (Est.)  : {sortino_ratio:.2f}") # <-- Affichage Sortino
        print("-" * 50)
